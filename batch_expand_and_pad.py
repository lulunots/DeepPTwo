"""Batch driver for expand_and_pad.py.
...
from expand_and_pad import _INTERPOLATION_CHOICES, _parse_color, expand_and_pad
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="batch_expand_and_pad.py",
        description=(
            "Resize+pad every .ome.tif file in a folder to a square canvas, "
            "in parallel, within a single Python process."
        ),
        epilog=(
            "Example:\n"
            "  python batch_expand_and_pad.py /path/to/input_folder 214 /path/to/outloc --processes 2\n"
        ),
    ...
    parser.add_argument(
        ...
        default="cubic",
        help="Interpolation method used when resizing. Default: cubic (good for upscaling).",
    )
    parser.add_argument(
        "-v", "--verbose",
        action="store_true",
